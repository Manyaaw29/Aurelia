...
async function loadRecommendedProducts() {
    try {
        // Product names to fetch (exactly 4 products)
        const productNames = [
        ...
        if (data.success && data.products) {
            // Filter for the specific products and maintain order
            ...
            // Render the products (only image and name, no price)
            if (recommendedProducts.length > 0) {
                recommendedContainer.innerHTML = recommendedProducts.map(product => {
                    ...
                    return `
                        <a href="/product/${product._id}" class="recommended-item">
                            <img src="${imageUrl}" alt="${product.name}" class="recommended-image" onerror="this.src='/images/placeholder.jpg'">
                ...
                recommendedContainer.innerHTML = '<p style="color: #ccc; text-align: center; padding: 20px;">Loading recommendations...</p>';
            }
        }
    } catch (error) {
        ...
        if (container) {
            container.innerHTML = '<p style="color: #ccc; text-align: center; padding: 20px;">Unable to load recommendations</p>';
        }
    }
}
